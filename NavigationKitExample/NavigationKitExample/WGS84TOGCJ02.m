...
{
    CLLocationCoordinate2D fixed = location;
    
    // Are we in Beijing or not?
    if (location.latitude > 39.2 && location.latitude < 41.0 && location.longitude > 115.4 && location.longitude < 118.0)
    {
        // There should be SOME offset to counter the bug
        fixed.latitude -= 0.0023;
        fixed.longitude -= 0.0065;
    }
